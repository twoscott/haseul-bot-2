...
var guildCommand = &router.Command{
	Name:      "guild",
	Aliases:   []string{"ginfo", "guildinfo", "server", "sinfo", "serverinfo"},
	UseTyping: true,
	Run:       guildRun,
}

func guildRun(ctx router.CommandCtx, args []string) {
...
func guildEmbed(ctx *router.CommandCtx, guild *discord.Guild) *discord.Embed {
	...
	if guild.Icon != "" {
		iconUploaded, _ = util.ImgUploadTime(guild.IconURL())
	}
